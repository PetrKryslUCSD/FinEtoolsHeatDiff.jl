...
function steel_block_m()
	...
	lx  = selectnode(fens; box=[dimensions[1] dimensions[1] -Inf Inf -Inf Inf],  inflate = tolerance)
	ly  = selectnode(fens; box=[-Inf Inf dimensions[2] dimensions[2] -Inf Inf],  inflate = tolerance)
	lz  = selectnode(fens; box=[-Inf Inf -Inf Inf dimensions[3] dimensions[3]],  inflate = tolerance)
	...
	@gp  :- vec(ts) vec(center_T) "lw 2 lc rgb 'red' with lines title 'Temperature at the corner' " 
